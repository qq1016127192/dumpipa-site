<template>
  <div class="coin-orders-container">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="order in orders" :key="order.id">
              ...
              <td class="amount">{{ order.coins || 0 }}</td>
              <td class="bonus">{{ order.bonus_coins || 0 }}</td>
...
import { getAllCoinOrders } from '@/api/vipCoin'
...
const loadOrders = async (page: number = 1) => {
  try {
    ...
    const res = await getAllCoinOrders(params)
    if (res.ok === 1) {
